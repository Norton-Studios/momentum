...
const CreateTenantSchema = z.object({
  name: z.string().min(1).regex(/^[a-zA-Z0-9-_]+$/, 'Name must be alphanumeric with hyphens and underscores only'),
  adminEmail: z.string().email(),
})

const UpdateTenantSchema = z.object({
  name: z.string().min(1).regex(/^[a-zA-Z0-9-_]+$/, 'Name must be alphanumeric with hyphens and underscores only').optional(),
})
...
router.get('/tenants/:tenantId/configs', async (req, res) => {
  const db = req.app.get('db') as PrismaClient
  const { tenantId } = req.params
...
router.post('/tenants/:tenantId/configs', async (req, res) => {
  const db = req.app.get('db') as PrismaClient
  const { tenantId } = req.params
...
router.put('/tenants/:tenantId/configs/:dataSource/:key', async (req, res) => {
  const db = req.app.get('db') as PrismaClient
...
router.delete('/tenants/:tenantId/configs/:dataSource/:key', async (req, res) => {
  const db = req.app.get('db') as PrismaClient
  const { tenantId, dataSource, key } = req.params
